...
PatchTess ConstantHS(InputPatch<VS_OUT, 3> input, int patchID : SV_PrimitiveID)
{
	...
	output.edgeTess[0] = 1;
	output.edgeTess[1] = 2;
	output.edgeTess[2] = 3;		// 여기 edgeTess를 바꿔주면, 정점의 갯수가 바뀌고, 구조가 바뀐다. 갯수가 늘어나면 좀 더 세세해진다는 특징이 있다.
	output.insideTess = 1;		// 멀리가게 되어 보이지 않는 부분에 대해 부하를 줄이고 싶다면, 이 부분의 갯수를 작게 해주고, 
										// 가까이 있을때 세세하게 보여주고 싶으면 정점의 갯수를 늘려주면 된다.
	return output;
}
