...
PatchTess ConstantHS(InputPatch<VS_OUT, 3> input, int patchID : SV_PrimitiveID)
{
	...
	float minDistance = g_vec2_1.x;		// 카메라의 터레인간의 최소 거리 (가장 가까운 거리)		- 가장 큰 테셀레이션 레벨
	float maxDistance = g_vec2_1.y;		  // 카메라와 터레인간의 최대 거리 (가장 먼 거리)			- 가장 작은 테셀레이션 레벨

	float3 edge0Pos = (input[1].pos + input[2].pos) / 2.f;
	float3 edge1Pos = (input[2].pos + input[0].pos) / 2.f;
	float3 edge2Pos = (input[0].pos + input[1].pos) / 2.f;			// 엣지는 꼭짓점이 바라보는 점임. 그러므로 꼭짓점을 제외한 나머지 두놈의 평균값으로 구해놓은것
	...
	output.edgeTess[2] = edge2TessLevel;		//테셀레이션 레벨 설정 - 카메라와의 거리로 구해줌.
...
DS_OUT DS_Main(const OutputPatch<HS_OUT, 3> input, float3 location : SV_DomainLocation, PatchTess patch)
{
	...
	float2 fullUV = float2(uv.x / (float)tileCountX, uv.y / (float)tileCountZ);		// 0~64 -> 0 ~ 1로 uv좌표 재계산
	...
	output.viewNormal = normalize(cross(output.viewBinormal, output.viewTangent));//탄젠트 바이노말 외적 -> 노말
