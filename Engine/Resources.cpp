...
shared_ptr<Mesh> Resources::LoadCubeMesh()
{
	...
	float w2 = 0.5f;	// 전체 width/2
	float h2 = 0.5f;	// 전체 height/2
	float d2 = 0.5f;  // 전체 depth/2
...
shared_ptr<Mesh> Resources::LoadSphereMesh()
{
	...
	uint32 stackCount = 20; // 가로 분할		 늘릴수록 더 원에 가까워짐. 대신 부하도 늘어남.
	...
	// 북극
	v.pos = Vec3(0.0f, radius, 0.0f); // 구의 반지름만큼 위로 간 것
	v.uv = Vec2(0.5f, 0.0f);
	v.normal = v.pos;
	v.normal.Normalize();
	...
	// 남극
	v.pos = Vec3(0.0f, -radius, 0.0f);		// 구의 반지름만큼 밑으로 간 것
	v.uv = Vec2(0.5f, 1.0f);
	v.normal = v.pos;
	v.normal.Normalize();
...
void Resources::CreateDefaultMaterial()
{
	...
	//Terrain
	{
		...
		shared_ptr<Texture> texture = GET_SINGLE(Resources)->Load<Texture>(L"Terrain", L"..\\Resources\\Texture\\Terrain\\texture.jpg");
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
		material->SetTexture(0, texture);
